...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!name) return NextResponse.json({ error: 'Нужно название объекта' }, { status: 400 })
    ...
    if (error) return NextResponse.json({ error: error.message || 'Не смогла создать объект' }, { status: 500 })
