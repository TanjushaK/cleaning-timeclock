...
export default function AdminApprovalsPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    setEditById((prev) => {
      ...
      for (const w of list) {
        if (!w?.id) continue
        if (!next[w.id]) next[w.id] = { phone: w.phone || '', email: w.email || '' }
        else {
          // мягко синхроним, но не ломаем то, что админ уже набрал
          if (!String(next[w.id].phone || '').trim()) next[w.id].phone = w.phone || ''
          if (!String(next[w.id].email || '').trim()) next[w.id].email = w.email || ''
        }
      }
      return next
    })
  ...
  const saveContacts = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        if (phone && !isE164(phone)) throw new Error('Телефон должен быть в формате E.164, например +31612345678')

        await authFetchJson(`/api/admin/workers/${encodeURIComponent(id)}/profile`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ phone: phone || null, email: em || null }),
        })
                ...
                <div key={w.id} className="rounded-2xl border border-amber-500/20 bg-zinc-950/60 p-4">
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                    <label className="grid gap-1">
                      <span className="text-[11px] opacity-70">Телефон (E.164)</span>
                      <input
                        className="w-full rounded-xl bg-zinc-900/60 border border-amber-500/20 px-3 py-2 text-sm outline-none focus:border-amber-400/50"
