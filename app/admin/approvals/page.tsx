...
export default function AdminApprovalsPage() {
  const BG = 'bg-[#0b0604]'
  const CARD = 'border border-amber-500/20 bg-[#120806]/70 shadow-[0_0_0_1px_rgba(245,158,11,0.06),0_30px_80px_rgba(0,0,0,0.55)] backdrop-blur'
        ...
        <div className={`rounded-3xl ${CARD} p-5 sm:p-6`}>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-wrap gap-2 sm:justify-end">
              ...
              <button className={BTN} disabled={busy} onClick={() => void refresh().then(() => setNotice('Обновлено.')).catch((e) => setError(String((e as any)?.message || e || 'Ошибка')))}>
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                      <label className="grid gap-1">
                        ...
                        <input
                          ...
                          value={edit.phone}
                          onChange={(e) =>
                            setEditById((p) => ({ ...p, [w.id]: { ...(p[w.id] || { phone: '', email: '' }), phone: e.target.value } }))
                          }
                          disabled={busy}
                        />
                      </label>
                      ...
                      <label className="grid gap-1">
                        ...
                        <input
                          ...
                          value={edit.email}
                          onChange={(e) =>
                            setEditById((p) => ({ ...p, [w.id]: { ...(p[w.id] || { phone: '', email: '' }), email: e.target.value } }))
                          }
                          disabled={busy}
                        />
                      </label>
