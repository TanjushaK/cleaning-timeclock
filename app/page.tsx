        ...
        <div className="w-full max-w-md rounded-2xl border border-amber-500/20 bg-zinc-950/60 p-6 shadow-xl">
          ...
          {loginMode === "email" ? (
            <div className="mt-4 space-y-3">
              ...
              <button className="w-full rounded-xl bg-amber-500 text-zinc-950 px-4 py-2 text-sm font-semibold hover:bg-amber-400 disabled:opacity-60" onClick={doLoginEmail} disabled={busy || !email.trim() || !password.trim()}>
                {busy ? "Вхожу…" : "Войти"}
              </button>
            </div>
          ) : (
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex gap-2">
            {me.profile?.role === "admin" ? (
              <a className="rounded-xl border border-amber-500/30 px-3 py-2 text-sm hover:bg-amber-500/10" href="/admin">Админка</a>
            ) : null}
            <button
              ...
              onClick={async () => {
                ...
                catch (e: any) { setError(String(e?.message || e || "Ошибка обновления")); }
