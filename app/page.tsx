...
const loadAll = useCallback(async () => {
  ...
  const doStart = useCallback(
    async (jobId: string) => {
      ...
      // IMPORTANT: no optimistic "in_progress".
      // Start is allowed ONLY after server confirms GPS accuracy + distance.
      ...
      try {
        ...
        // Offline START is forbidden: without server validation it becomes a bypass.
        if (isOfflineishError(e)) {
          setError("Нет сети. Старт запрещён без проверки GPS. Подключись к интернету.");
        } else {
          setError(String(e?.message || e));
        }
      } finally {
