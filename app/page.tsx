...
export default function AppPage() {
  ...
  const doPhoneSend = useCallback(async () => {
    ...
    try {
      const ph = phone.trim();
      if (!ph) throw new Error("Введи телефон");
      if (!ph.startsWith("+")) throw new Error("Телефон нужен в формате E.164, например +31612345678");
  ...
  const doPhoneVerify = useCallback(async () => {
    ...
    try {
      ...
      if (newPw.length < 8) throw new Error("Пароль минимум 8 символов");
      ...
      setNotice("Пароль обновлён. Войди по паролю.");
      setLoginMode("email");
      setOtpSent(false);
      setOtp("");
      setRecoverNewPassword("");
    } catch (e: any) {
      setError(String(e?.message || e || "Ошибка подтверждения"));
    } finally {
      setBusy(false);
    }
  }, [phone, otp, recoverNewPassword]);
