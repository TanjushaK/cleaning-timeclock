...
const PHRASES: PhraseRow[] = [
  ...
  row(
    "Объект + дата + время + несколько работников.",
    "Об'єкт + дата + час + кілька працівників.",
    "Site + date + time + several workers.",
    "Locatie + datum + tijd + meerdere medewerkers.",
  ),
  row("Работники (можно несколько)", "Працівники (можна кілька)", "Workers (multiple allowed)", "Medewerkers (meerdere mogelijk)"),
  row("Конец", "Кінець", "End", "Einde"),
  row("От", "Від", "From", "Van"),
  row("До", "До", "To", "Tot"),
  row(
    "Tanija • объекты • работники • смены",
    "Tanija • об'єкти • працівники • зміни",
    "Tanija • sites • workers • shifts",
    "Tanija • locaties • medewerkers • diensten",
  ),
  row("Работник", "Працівник", "Worker", "Medewerker"),
  row("Все", "Усе", "All", "Alle"),
  row("Нет смен", "Немає змін", "No shifts", "Geen diensten"),
  row("Править", "Правити", "Edit", "Bewerken"),
  row("Принято", "Прийнято", "Accepted", "Geaccepteerd"),
  row("Дата", "Дата", "Date", "Datum"),
  row("Время", "Час", "Time", "Tijd"),
  row("Объект", "Об'єкт", "Site", "Locatie"),
  row("Начал", "Почав", "Started", "Gestart"),
  row("Закончил", "Закінчив", "Finished", "Beëindigd"),
  row("Статус", "Статус", "Status", "Status"),
  row("Объект:", "Об'єкт:", "Site:", "Locatie:"),
  row("Работник:", "Працівник:", "Worker:", "Medewerker:"),
