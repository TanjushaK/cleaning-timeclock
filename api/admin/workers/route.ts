...
export async function GET(req: Request) {
  try {
    ...
    const { data: profs, error } = await supabase
      .from('profiles' 'id, role, full_name, phone, active, avatar_url' 'role', 'worker' 'full_name', { ascending: true })
...
export async function POST(req: Request) {
  try {
    ...
    const email = typeof body?.email === 'string' ? body.email.trim() : '' 'string' ? body.password.trim() : '' 'string' ? body.full_name.trim() : '' 'string' ? body.phone.trim() : '' 'email_required' 'password_min_6')
